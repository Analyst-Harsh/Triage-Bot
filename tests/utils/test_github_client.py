"""Tests for `utils.github_client`: `GitHubClient`'s read/write GitHub
operations and the `get_github_client()` singleton factory.

`GitHubClient.__init__` is real production construction (builds its own
`Github` from `Settings`, plus its own `DiffApplier` -- no test-only
constructor parameter). Tests that need a fake substitute
`_FakeGitHubClient`, a private subclass overriding `__init__` to accept an
already-faked `Github` directly, mirroring the
`_FakePlannerNode`/`_FakeRiskCheckNode`-style test doubles in
`tests/graph/nodes/conftest.py`.

Every PyGithub object below is a `create_autospec(..., spec_set=True)` fake
rather than a hand-rolled stand-in — this repo has no network access in
tests, and autospec fails loudly (AttributeError) if these fakes drift from
PyGithub's real attribute surface, while `create_autospec`'s `-> Any` return
type keeps strict pyright satisfied without a single `# pyright: ignore`.
"""

from datetime import UTC, datetime
from typing import Any
from unittest.mock import create_autospec

import pytest
from github import Auth, Github, GithubException
from github.ContentFile import ContentFile
from github.GitCommit import GitCommit
from github.GitRef import GitRef
from github.GitTree import GitTree
from github.Issue import Issue
from github.IssueComment import IssueComment
from github.Label import Label
from github.NamedUser import NamedUser
from github.PullRequest import PullRequest
from github.Repository import Repository
from pydantic import SecretStr

from config.settings import Settings
from graph.schemas import IssueSource
from utils.diff_applier import AppliedFile, DiffApplier, DiffApplyError
from utils.github_client import (
    GitHubClient,
    _tree_element,  # pyright: ignore[reportPrivateUsage]
    get_github_client,
)


@pytest.fixture(autouse=True)
def clear_github_client_singleton() -> None:
    """`get_github_client()` is `@lru_cache`d for real singleton behavior in
    production, but that means tests exercising different `Settings`
    combinations must each start from a clean cache, or they'd silently
    observe a stale instance from an earlier test."""
    get_github_client.cache_clear()


class _FakeGitHubClient(GitHubClient):
    """Test double: overrides `GitHubClient.__init__` (which otherwise
    builds a real `Github` from `Settings` and a real `DiffApplier`) to
    accept an already-faked `Github` directly -- the real fetch/post/apply/
    close logic (inherited, not overridden) is what's actually under test.
    `DiffApplier` itself isn't faked here: tests that need to control diff
    application instead patch `DiffApplier.apply` directly (see
    `_fake_apply_diff` usages below), since a real `DiffApplier()` is cheap
    to construct (no I/O until `.apply()` is actually called)."""

    def __init__(self, github: Github) -> None:
        self._github = github
        self._diff_applier = DiffApplier()


def make_fake_issue(**overrides: Any) -> Any:
    """Returns `Any`, not `Issue`: callers need Mock-specific introspection
    (`.assert_called_once_with`/`.side_effect`) that a real `Issue`'s type
    stub doesn't expose -- the same deliberate `Any` exception AGENTS.md
    carves out for test-fixture helpers."""
    fake_user = create_autospec(NamedUser, instance=True, spec_set=True)
    fake_user.login = "octocat"

    fake_label = create_autospec(Label, instance=True, spec_set=True)
    fake_label.name = "bug"

    fake_comment = create_autospec(IssueComment, instance=True, spec_set=True)
    fake_comment.html_url = "https://github.com/octocat/Hello-World/issues/1#issuecomment-1"

    defaults: dict[str, Any] = {
        "number": 1,
        "title": "Found a bug",
        "body": "This is a bug report.",
        "user": fake_user,
        "author_association": "NONE",
        "labels": [fake_label],
        "created_at": datetime(2011, 4, 22, tzinfo=UTC),
        "html_url": "https://github.com/octocat/Hello-World/issues/1",
    }
    defaults.update(overrides)

    fake_issue = create_autospec(Issue, instance=True, spec_set=True)
    for attr, value in defaults.items():
        setattr(fake_issue, attr, value)
    fake_issue.create_comment.return_value = fake_comment
    return fake_issue


def make_fake_client(issue: Issue) -> Github:
    fake_repo = create_autospec(Repository, instance=True, spec_set=True)
    fake_repo.get_issue.return_value = issue

    fake_client = create_autospec(Github, instance=True, spec_set=True)
    fake_client.get_repo.return_value = fake_repo
    return fake_client


def make_client(issue: Issue) -> GitHubClient:
    return _FakeGitHubClient(make_fake_client(issue))


def make_fake_repo() -> Any:
    return create_autospec(Repository, instance=True, spec_set=True)


def make_fake_content_file(content: str) -> Any:
    fake_content_file = create_autospec(ContentFile, instance=True, spec_set=True)
    fake_content_file.decoded_content = content.encode("utf-8")
    return fake_content_file


def make_repo_client(repo: Any) -> GitHubClient:
    fake_client = create_autospec(Github, instance=True, spec_set=True)
    fake_client.get_repo.return_value = repo
    return _FakeGitHubClient(fake_client)


def make_fake_pull_request(url: str = "https://github.com/octocat/Hello-World/pull/1") -> Any:
    fake_pr = create_autospec(PullRequest, instance=True, spec_set=True)
    fake_pr.html_url = url
    return fake_pr


def test_fetch_issue_maps_core_fields() -> None:
    client = make_client(make_fake_issue())

    payload = client.fetch_issue("octocat/Hello-World", 1)

    assert payload.repo_full_name == "octocat/Hello-World"
    assert payload.issue_number == 1
    assert payload.title == "Found a bug"
    assert payload.body == "This is a bug report."
    assert payload.author == "octocat"
    assert payload.author_association == "NONE"
    assert payload.labels == ["bug"]
    assert payload.created_at == datetime(2011, 4, 22, tzinfo=UTC)
    assert payload.url == "https://github.com/octocat/Hello-World/issues/1"


def test_fetch_issue_sets_source_to_replay() -> None:
    client = make_client(make_fake_issue())

    payload = client.fetch_issue("octocat/Hello-World", 1)

    assert payload.source is IssueSource.REPLAY


def test_fetch_issue_leaves_installation_id_none() -> None:
    client = make_client(make_fake_issue())

    payload = client.fetch_issue("octocat/Hello-World", 1)

    assert payload.installation_id is None


def test_fetch_issue_coerces_none_body_to_empty_string() -> None:
    """GitHub's API can return `body: null`; IssuePayload.body is a required str."""
    client = make_client(make_fake_issue(body=None))

    payload = client.fetch_issue("octocat/Hello-World", 1)

    assert payload.body == ""


def test_fetch_issue_maps_multiple_labels_by_name() -> None:
    label_a = create_autospec(Label, instance=True, spec_set=True)
    label_a.name = "bug"
    label_b = create_autospec(Label, instance=True, spec_set=True)
    label_b.name = "help wanted"
    client = make_client(make_fake_issue(labels=[label_a, label_b]))

    payload = client.fetch_issue("octocat/Hello-World", 1)

    assert payload.labels == ["bug", "help wanted"]


def test_post_comment_creates_comment_and_returns_url() -> None:
    issue = make_fake_issue()
    client = make_client(issue)

    url = client.post_comment("octocat/Hello-World", 1, "Thanks for the report!")

    issue.create_comment.assert_called_once_with("Thanks for the report!")
    assert url == "https://github.com/octocat/Hello-World/issues/1#issuecomment-1"


def test_post_comment_propagates_github_exception() -> None:
    issue = make_fake_issue()
    issue.create_comment.side_effect = GithubException(500, {"message": "boom"}, None)
    client = make_client(issue)

    with pytest.raises(GithubException):
        client.post_comment("octocat/Hello-World", 1, "Thanks for the report!")


def test_apply_labels_adds_and_removes() -> None:
    issue = make_fake_issue()
    client = make_client(issue)

    client.apply_labels("octocat/Hello-World", 1, ["bug", "help wanted"], ["stale"])

    issue.add_to_labels.assert_called_once_with("bug", "help wanted")
    issue.remove_from_labels.assert_called_once_with("stale")


def test_apply_labels_skips_add_call_when_nothing_to_add() -> None:
    issue = make_fake_issue()
    client = make_client(issue)

    client.apply_labels("octocat/Hello-World", 1, [], ["stale"])

    issue.add_to_labels.assert_not_called()
    issue.remove_from_labels.assert_called_once_with("stale")


def test_apply_labels_propagates_github_exception() -> None:
    issue = make_fake_issue()
    issue.add_to_labels.side_effect = GithubException(500, {"message": "boom"}, None)
    client = make_client(issue)

    with pytest.raises(GithubException):
        client.apply_labels("octocat/Hello-World", 1, ["bug"], [])


def test_close_issue_posts_comment_then_closes() -> None:
    issue = make_fake_issue()
    client = make_client(issue)

    client.close_issue("octocat/Hello-World", 1, "Duplicate of #10.")

    issue.create_comment.assert_called_once_with("Duplicate of #10.")
    issue.edit.assert_called_once_with(state="closed")


def test_close_issue_without_comment_only_closes() -> None:
    issue = make_fake_issue()
    client = make_client(issue)

    client.close_issue("octocat/Hello-World", 1, None)

    issue.create_comment.assert_not_called()
    issue.edit.assert_called_once_with(state="closed")


def test_close_issue_propagates_github_exception() -> None:
    issue = make_fake_issue()
    issue.edit.side_effect = GithubException(500, {"message": "boom"}, None)
    client = make_client(issue)

    with pytest.raises(GithubException):
        client.close_issue("octocat/Hello-World", 1, None)


def test_get_github_client_anonymous_when_no_token_configured(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("utils.github_client.get_settings", lambda: Settings(github_token=None))

    client = get_github_client()

    assert client.raw.requester.auth is None


def test_get_github_client_authenticates_with_settings_github_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "utils.github_client.get_settings",
        lambda: Settings(github_token=SecretStr("settings-token")),
    )

    client = get_github_client()

    auth = client.raw.requester.auth
    assert isinstance(auth, Auth.Token)
    assert auth.token == "settings-token"


def test_get_github_client_is_a_singleton(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("utils.github_client.get_settings", lambda: Settings(github_token=None))

    first = get_github_client()
    second = get_github_client()

    assert first is second


def test_tree_element_for_modified_file_uses_inline_content() -> None:
    element = _tree_element(AppliedFile(path="a.py", content="new content\n"))

    # PyGithub's `InputGitTreeElement` exposes no public accessor for its
    # fields -- `_identity` (the property it uses to build the API payload)
    # is the only way to verify what was actually constructed.
    assert element._identity == {  # pyright: ignore[reportPrivateUsage]
        "path": "a.py",
        "mode": "100644",
        "type": "blob",
        "content": "new content\n",
    }


def test_tree_element_for_deleted_file_uses_null_sha() -> None:
    element = _tree_element(AppliedFile(path="a.py", content=None))

    assert element._identity == {  # pyright: ignore[reportPrivateUsage]
        "path": "a.py",
        "mode": "100644",
        "type": "blob",
        "sha": None,
    }


def test_create_pull_request_from_diff_builds_commit_and_returns_pr_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo = make_fake_repo()
    repo.get_contents.return_value = make_fake_content_file("old content\n")

    fake_base_tree = create_autospec(GitTree, instance=True, spec_set=True)
    fake_base_commit = create_autospec(GitCommit, instance=True, spec_set=True)
    fake_base_commit.tree = fake_base_tree
    repo.get_git_commit.return_value = fake_base_commit

    fake_tree = create_autospec(GitTree, instance=True, spec_set=True)
    repo.create_git_tree.return_value = fake_tree

    fake_new_commit = create_autospec(GitCommit, instance=True, spec_set=True)
    fake_new_commit.sha = "newcommitsha"
    repo.create_git_commit.return_value = fake_new_commit

    fake_ref = create_autospec(GitRef, instance=True, spec_set=True)
    repo.create_git_ref.return_value = fake_ref

    fake_pr = create_autospec(PullRequest, instance=True, spec_set=True)
    fake_pr.html_url = "https://github.com/octocat/Hello-World/pull/7"
    repo.create_pull.return_value = fake_pr

    applied = [AppliedFile(path="a.py", content="new content\n")]

    def _fake_apply(
        _self: DiffApplier, _diff: str, _base_files: dict[str, str | None]
    ) -> list[AppliedFile]:
        return applied

    monkeypatch.setattr(DiffApplier, "apply", _fake_apply)

    client = make_repo_client(repo)

    url = client.create_pull_request_from_diff(
        "octocat/Hello-World",
        diff="--- a/a.py\n+++ b/a.py\n",
        target_files=["a.py"],
        base_commit_sha="basecommitsha",
        base_branch="main",
        branch_name="triage-bot/issue-1-abcd1234",
        title="Fix the bug",
        body="Fixes #1",
    )

    repo.get_git_commit.assert_called_once_with("basecommitsha")
    repo.create_git_tree.assert_called_once()
    _, tree_kwargs = repo.create_git_tree.call_args
    assert tree_kwargs["base_tree"] is fake_base_tree
    repo.create_git_commit.assert_called_once_with(
        message="Fix the bug", tree=fake_tree, parents=[fake_base_commit]
    )
    repo.create_git_ref.assert_called_once_with(
        "refs/heads/triage-bot/issue-1-abcd1234", "newcommitsha"
    )
    repo.create_pull.assert_called_once_with(
        base="main", head="triage-bot/issue-1-abcd1234", title="Fix the bug", body="Fixes #1"
    )
    assert url == "https://github.com/octocat/Hello-World/pull/7"


def test_create_pull_request_from_diff_fetches_each_target_file_at_base_commit_sha(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo = make_fake_repo()
    repo.get_contents.return_value = make_fake_content_file("content\n")
    repo.get_git_commit.return_value = create_autospec(GitCommit, instance=True, spec_set=True)
    repo.create_git_tree.return_value = create_autospec(GitTree, instance=True, spec_set=True)
    repo.create_git_commit.return_value = create_autospec(GitCommit, instance=True, spec_set=True)
    repo.create_pull.return_value = make_fake_pull_request()

    captured_base_files: dict[str, str | None] = {}

    def _fake_apply(
        _self: DiffApplier, _diff: str, base_files: dict[str, str | None]
    ) -> list[AppliedFile]:
        captured_base_files.update(base_files)
        return [AppliedFile(path=p, content="x") for p in base_files]

    monkeypatch.setattr(DiffApplier, "apply", _fake_apply)
    client = make_repo_client(repo)

    client.create_pull_request_from_diff(
        "octocat/Hello-World",
        diff="diff",
        target_files=["a.py", "b.py"],
        base_commit_sha="basecommitsha",
        base_branch="main",
        branch_name="triage-bot/issue-1-abcd1234",
        title="Fix",
        body="Body",
    )

    assert repo.get_contents.call_count == 2
    repo.get_contents.assert_any_call("a.py", ref="basecommitsha")
    repo.get_contents.assert_any_call("b.py", ref="basecommitsha")
    assert captured_base_files == {"a.py": "content\n", "b.py": "content\n"}


def test_create_pull_request_from_diff_treats_404_as_new_file(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo = make_fake_repo()
    repo.get_contents.side_effect = GithubException(404, {"message": "Not Found"}, None)
    repo.get_git_commit.return_value = create_autospec(GitCommit, instance=True, spec_set=True)
    repo.create_git_tree.return_value = create_autospec(GitTree, instance=True, spec_set=True)
    repo.create_git_commit.return_value = create_autospec(GitCommit, instance=True, spec_set=True)
    repo.create_pull.return_value = make_fake_pull_request()

    captured_base_files: dict[str, str | None] = {}

    def _fake_apply(
        _self: DiffApplier, _diff: str, base_files: dict[str, str | None]
    ) -> list[AppliedFile]:
        captured_base_files.update(base_files)
        return [AppliedFile(path="new.py", content="new\n")]

    monkeypatch.setattr(DiffApplier, "apply", _fake_apply)
    client = make_repo_client(repo)

    client.create_pull_request_from_diff(
        "octocat/Hello-World",
        diff="diff",
        target_files=["new.py"],
        base_commit_sha="basecommitsha",
        base_branch="main",
        branch_name="triage-bot/issue-1-abcd1234",
        title="Fix",
        body="Body",
    )

    assert captured_base_files == {"new.py": None}


def test_create_pull_request_from_diff_propagates_github_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo = make_fake_repo()
    repo.get_contents.return_value = make_fake_content_file("content\n")
    repo.get_git_commit.return_value = create_autospec(GitCommit, instance=True, spec_set=True)
    repo.create_git_tree.side_effect = GithubException(500, {"message": "boom"}, None)

    def _fake_apply(
        _self: DiffApplier, _diff: str, _base_files: dict[str, str | None]
    ) -> list[AppliedFile]:
        return [AppliedFile(path="a.py", content="x")]

    monkeypatch.setattr(DiffApplier, "apply", _fake_apply)
    client = make_repo_client(repo)

    with pytest.raises(GithubException):
        client.create_pull_request_from_diff(
            "octocat/Hello-World",
            diff="diff",
            target_files=["a.py"],
            base_commit_sha="basecommitsha",
            base_branch="main",
            branch_name="triage-bot/issue-1-abcd1234",
            title="Fix",
            body="Body",
        )

    repo.create_git_ref.assert_not_called()
    repo.create_pull.assert_not_called()


def test_create_pull_request_from_diff_propagates_diff_apply_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo = make_fake_repo()
    repo.get_contents.return_value = make_fake_content_file("content\n")

    def _raising_apply(
        _self: DiffApplier, _diff: str, _base_files: dict[str, str | None]
    ) -> list[AppliedFile]:
        raise DiffApplyError("diff failed to apply: context mismatch")

    monkeypatch.setattr(DiffApplier, "apply", _raising_apply)
    client = make_repo_client(repo)

    with pytest.raises(DiffApplyError):
        client.create_pull_request_from_diff(
            "octocat/Hello-World",
            diff="diff",
            target_files=["a.py"],
            base_commit_sha="basecommitsha",
            base_branch="main",
            branch_name="triage-bot/issue-1-abcd1234",
            title="Fix",
            body="Body",
        )

    repo.get_git_commit.assert_not_called()


def test_create_pull_request_from_diff_wraps_non_utf8_base_content() -> None:
    repo = make_fake_repo()
    fake_content_file = create_autospec(ContentFile, instance=True, spec_set=True)
    fake_content_file.decoded_content = b"\xff\xfe"
    repo.get_contents.return_value = fake_content_file
    client = make_repo_client(repo)

    with pytest.raises(DiffApplyError, match="UTF-8"):
        client.create_pull_request_from_diff(
            "octocat/Hello-World",
            diff="diff",
            target_files=["a.py"],
            base_commit_sha="basecommitsha",
            base_branch="main",
            branch_name="triage-bot/issue-1-abcd1234",
            title="Fix",
            body="Body",
        )


def test_create_pull_request_from_diff_rejects_directory_path() -> None:
    repo = make_fake_repo()
    repo.get_contents.return_value = [make_fake_content_file("x\n")]
    client = make_repo_client(repo)

    with pytest.raises(DiffApplyError, match="directory"):
        client.create_pull_request_from_diff(
            "octocat/Hello-World",
            diff="diff",
            target_files=["a_dir"],
            base_commit_sha="basecommitsha",
            base_branch="main",
            branch_name="triage-bot/issue-1-abcd1234",
            title="Fix",
            body="Body",
        )
