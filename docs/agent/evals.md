...
- Prompt-injection golden cases — none of this repo's real replay/test issues happen to contain a genuine injection attempt; adding one requires actually running a crafted issue through the pipeline first (real API cost) so it gets traced, not fabricating trace data.
